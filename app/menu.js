...
async function checkForUpdate(url) {
    ...
    if (+localInstallerVersion < +serverInstallerVersion) {
        ...
        for (let i = 0; i < serverJSON.installerUpdateContent.length; i++) {
            ...
            if (version <= localInstallerVersion)
                break;
            updateStr += `
...
localStorage.getItem('autoCheckUpdates') == 'true' && checkForUpdate(checkUrl);
...
function createIframe() {
    ...
    win.loadURL(`file://${__dirname}/../doc/(必看)无名杀全教程10.0.pdf`);
    win.on('closed', () => {
        win = null
    });
}
    ...
    request.onsuccess = function (e) {
        ...
        store.get('extensions').onsuccess = function (e) {
            ...
            if (!Array.isArray(extensions) || extensions.length == 0) {
                config.put(['搬运自用', '应用配置', '拖拽读取', '在线更新'], 'extensions');
            }
        };
    };
}
