...
game.import("extension", function (lib, game, ui, get, ai, _status) {
    ...
    var extensionConf = [{
            ...
            desc: 'Github 原生',
            tip: '需要代理',
            url: 'https://raw.githubusercontent.com/libccy/noname-extension/master/',
        }];
    ...
    var extensionConf = initExtensionConf();

    const add = JSON.parse(localStorage.getItem('noname_github') || JSON.stringify(srcConf));
    const addExt = JSON.parse(localStorage.getItem('noname_extension_src') || JSON.stringify(extensionConf));

    const updateURLS = {};
    const translateItem = {};
    const tipsItem = {};
    add.forEach(item => {
        updateURLS[item.key] = item.url;
        translateItem[item.key] = item.name;
        tipsItem[item.key] = item.desc;
    });
    const extensionURLS = {};
    const extensionTranslateItem = {};
    const extensionTipsItem = {};

    addExt.forEach(item => {
        extensionURLS[item.key] = item.url;
        extensionTranslateItem[item.key] = item.name;
        extensionTipsItem[item.key] = item.desc;
    });
    const defaultKey = Object.keys(updateURLS)[0];
    ...
    function myFetch(url, options = {
            timeout: typeof game.getExtensionConfig('在线更新', 'timeout') == 'number' ? game.getExtensionConfig('在线更新', 'timeout') : 5000
        }) {
    ...
    return {
        ...
        content: function (config, pack) {
            if (game.getExtensionConfig('在线更新', 'rewriteUpdateButton')) {
                ...
                const interval = setInterval(() => {
                        if (typeof game.download != 'function' || typeof game.writeFile != 'function')
                        ...
                        if (active.length < 2)
                            return;
                        if (active[0].innerText != '其它' || active[1].innerText != '更新')
                            return;
                        /** @type Element */
                        // @ts-ignore
            ...
            function checkUpdate() {
                game.shijianGetUpdateFiles().then(({
                        update
                    }) => {
        ...
        precontent: function () {
            ...
            //lib.updateURLS = updateURLS;
            Object.assign(lib.updateURLS, updateURLS);
            Object.assign(lib.config.extension_sources, extensionURLS);
            ...
            //	fastgit: 'https://raw.kgithub.com/libccy/noname',
            ...
            if (!game.getExtensionConfig('在线更新', 'update_link')) {
                game.saveConfig('update_link', defaultKey);
                game.saveExtensionConfig('在线更新', 'update_link', defaultKey);
                lib.updateURL = lib.updateURLS[defaultKey];
            } else {
            ...
            if (lib.configMenu.general.config.update_link) {
			 ...
			 if (lib.configMenu.general.config.extension_source) {
				 lib.configMenu.general.config.extension_source.item = extensionTranslateItem;
			 }

            /** 检测最快连接到的更新源  */
            // @ts-ignore
            game.getFastestUpdateURL = function (updateURLS = lib.updateURLS, translate = translateItem) {
                if (typeof updateURLS != 'object')
                    throw new TypeError('updateURLS must be an object type');
                if (typeof translate != 'object')
                    throw new TypeError('translate must be an object type');
                const promises = [];
            ...
            if (!Array.isArray(lib.updateReady))
                lib.updateReady = [];
            if (!Array.isArray(lib.updateAssetReady))
                lib.updateAssetReady = [];
            ...
            checkForAssetUpdate: {
                ...
                onclick: async function () {
                    ...
                    if (button.disabled) {
                        ...
                        button.innerHTML = '正在检查更新';
                        button.disabled = true;
                        game.shijianGetUpdateAssets().then(({
                                assets: updates,
                                skins
                            }) => {
                            delete window.noname_asset_list;
